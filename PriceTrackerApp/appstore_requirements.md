...
> Zorunlu ve eksik gereksinimlerin kontrol listesi.
...
AdMob reklamları IDFA kullanarak kişiselleştirilmiş reklam gösterebilir. Bu durumda ATT izni zorunludur. Şu anda uygulamada ATT hiç implemente edilmemiştir.

### Yapılması gerekenler:

- [ ] **Info.plist'e `NSUserTrackingUsageDescription` ekleyin:**
  ```xml
  <key>NSUserTrackingUsageDescription</key>
  <string>Size özel reklamlar gösterebilmek için cihaz tanımlayıcınızı kullanmak istiyoruz.</string>
  ```

- [ ] **AppTrackingTransparency framework'ünü import edin:**
  ```swift
  import AppTrackingTransparency
  ```

- [ ] **Uygulama başlangıcında ATT izni isteyin** (`PriceTrackerApp.swift`, `scenePhase .active` içinde):
  ```swift
  if #available(iOS 14, *) {
...
- [ ] **PrivacyInfo.xcprivacy içinde `NSPrivacyTracking`'i `true` yapın:**
  ```xml
...
`project.yml` içinde `IAP_PREMIUM_PRODUCT_ID: com.algorix.pricetracker.premium` tanımlı ve README'de premium üyelikten bahsediliyor, ancak StoreKit satın alma kodu **hiç yazılmamış**.

### Yapılması gerekenler:

- [ ] **StoreKit 2 framework'ünü ekleyin:** `import StoreKit`
- [ ] **Ürün sorgulama:** `Product` nesnesi ile premium ürün bilgisini App Store Connect'dan çekin
- [ ] **Satın alma akışı:** `Product.purchase()` çağrısı ile ödeme işlemini başlatın
- [ ] **Kullanıcı arayüzü:** `SettingsView.swift`'e "Premium'a Yükselt" butonu ve premium durum göstergesi ekleyin
- [ ] **Özellik kısıtlama:** Premium olmayan kullanıcılar için limit (örn. ürün sayısı sınırı) uygulayın
...
## 3. Background Modes — Eksik Implementasyon

**Guideline:** 2.5.1 — Software Requirements / 2.5.4 — Background Tasks
**Durum:** ❌ Eksik

`Info.plist`'te `UIBackgroundModes: [fetch, processing]` tanımlanmış ve `BGTaskSchedulerPermittedIdentifiers` ayarlanmış ancak **hiçbir Swift dosyasında BGTaskScheduler kodu bulunmuyor**.

### Yapılması gerekenler:

- [ ] **`PriceTrackerApp.swift` içinde BGTaskScheduler kaydı yapın:**
  ```swift
...
- [ ] **Background task schedule fonksiyonu ekleyin:**
  ```swift
  func scheduleAppRefresh() {
      let request = BGAppRefreshTaskRequest(identifier: "com.algorix.pricetracker.pricecheck")
      request.earliestBeginDate = Date(timeIntervalSinceNow: intervalHours * 3600)
      try? BGTaskScheduler.shared.submit(request)
  }
  ```

- [ ] **Task complete yönetimi:** Task sonunda `task.setTaskCompleted(success: true/false)` çağrısı yapın

- [ ] **Alternatif:** Background modları kullanmayacaksanız `Info.plist`'ten kaldırın
...
## 5. Privacy Manifest Tutarsızlığı

**Guideline:** 5.1.1 — Privacy
**Durum:** ❌ Eksik / Tutarsız
...
**Guideline:** 2.1 — App Completeness, 4.2 — Minimum Functionality
**Durum:** ⚠️ Riskli
...
- [ ] **AppLogo 2x ve 3x görsellerini ekleyin:**
...
**Guideline:** 5.1.1(iv) — Account Deletion / Data Deletion
**Durum:** ✅ Mevcut / ⚠️ Kısmi

Uygulamada hesap oluşturma yok, bu nedenle "account deletion" zorunluluğu yok. Ancak:

### Yapılması gerekenler:

- [ ] **Kullanıcı verilerini tamamen silme:** LocalStorage (products.json) dosyası silinebilir olmalı → Settings'e "Tüm Verileri Sil" butonu ekleyin
...
| AdMob Production IDs | ❌ Eksik | 0/10 |
...
### Toplam: 68 / 160 = **%43**
*(Her kategori eşit ağırlıklı, 10 üzerinden puanlanmıştır)*

### Anlamı:
**%43 readiness** — Uygulama mevcut haliyle App Store'a gönderilirse **büyük olasılıkla reddedilir** (≈%90 ihtimal). Kritik 4 eksik (ATT, IAP, Background Tasks, AdMob ID) doğrudan rejection sebebidir.

### Minimum kabul edilebilir seviye: %75+
Bu seviyeye ulaşmak için kritik ve yüksek öncelikli maddelerin tamamı çözülmelidir.
